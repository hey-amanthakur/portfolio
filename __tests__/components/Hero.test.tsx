...
import { SECTION_LABELS, ROUTES } from '@/constants';
...
describe('Hero Component', (): void => {
  ...
  it('renders developer persona buttons', (): void => {
    render(<Hero />);
    expect(screen.getByRole('button', { name: 'Developer' })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'Off-hours' })).toBeInTheDocument();
  });
  ...
  it('renders CTA links for developer mode', (): void => {
    render(<Hero />);
    expect(screen.getByRole('link', { name: /Hire Me/ })).toHaveAttribute('href', ROUTES.contact);
  ...
  it('toggles to off-hours persona and shows food content', (): void => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Off-hours' }));

    expect(screen.getByText(/chase street food/)).toBeInTheDocument();
    expect(screen.getByText('Yeh Safar Swaad Ka')).toBeInTheDocument();
    expect(screen.getByRole('link', { name: /View food diary/ })).toHaveAttribute('href', ROUTES.instagram);
