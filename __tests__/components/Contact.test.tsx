...
describe('Contact Form Component', (): void => {
  it('renders standard cafe order form fields', (): void => {
    render(<Contact />);
    ...
    expect(screen.getByRole('button', { name: /Send Project Order/i })).toBeInTheDocument();
  });
  ...
  it('triggers form validators and shows errors for empty or invalid values', async (): Promise<void> => {
    ...
    const submitBtn = screen.getByRole('button', { name: /Send Project Order/i });
    ...
    expect(screen.getByText(/Please select a service course/i)).toBeInTheDocument();
  ...
  it('displays order success card after successful submission of valid inputs', async (): Promise<void> => {
    const user = userEvent.setup();
    render(<Contact />);
    ...
    await user.selectOptions(screen.getByLabelText(/Select Course/i), 'fullstack');
    await user.type(screen.getByLabelText(/Course Title/i), 'Build standard Spring boot backend');
    ...
    expect(screen.getByText(/Chef Aman has received your ingredients/i)).toBeInTheDocument();
  });
});
